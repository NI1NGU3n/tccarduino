    ...
    while (client.connected()) {//ENQUANTO EXISTIR CLIENTE CONECTADO, FAZ
      if (client.available()) { //SE O CLIENTE ESTÁ HABILITADO, FAZ
        ...
        if (c == '\n') { //SE ENCONTRAR "\n" É O FINAL DO CABEÇALHO DA REQUISIÇÃO HTTP, FAZ
          ...
          client.println("<center><font color='blue'><h1>ARDU-WATCH</font></center></h1>"); //ESCREVE "MASTERWALKER SHOP" NA PÁGINA
          client.println("<h1><center>CONTROLE DE BATIMENTOS CARDIACOS</center></h1>"); //ESCREVE "CONTROLE DE LED" NA PÁGINA
          ...
          /*  if (status == 1){ //SE VARIÁVEL FOR IGUAL A 1, FAZ
            ...
            .println("<center><font size='5'>Status atual do LED: </center>"); //ESCREVE "Status atual do LED:" NA PÁGINA
               if (status == 1){ //SE VARIÁVEL FOR IGUAL A 1, FAZ
